classdef Full3D_Euler < BREW.dynamics.DynamicsBase
    properties (Constant)
    ...
    methods
        function nextState = propagateState(obj, timestep, dt, state, u)
            ...
            rates_next = rates;
            nextState = [pos_next; vel_next; eul_next; rates_next] + obj.getInputMat(timestep, dt, state) * u;
        end
        ...
        function rotated_extent = rotate_extent(obj, dt, state, extent)
            % Rotate the extent by the full 3D rotation implied by the angular rates (p, q, r) over dt
            dphi = state(10) * dt;
            dtheta = state(11) * dt;
            dpsi = state(12) * dt;
            Rx = [1 0 0; 0 cos(dphi) -sin(dphi); 0 sin(dphi) cos(dphi)];
